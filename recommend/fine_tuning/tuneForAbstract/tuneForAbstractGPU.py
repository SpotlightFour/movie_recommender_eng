...
class GPUOptimizedLoRATrainer:
    """GPU优化的LoRA训练器 - 针对A800 GPU优化"""
    ...
    def train(self, output_dir=None):
        ...
        try:
            ...
            if output_dir is None:
                ...
                output_dir = f"./lora_results_abstract_{timestamp}"
            ...
            if torch.cuda.is_available():
                ...
                print(f"GPU内存清空后: {torch.cuda.memory_allocated() / 1024 ** 3:.2f} GB")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GPU优化的LoRA微调 - 简洁解释专用")
    parser.add_argument("--model_path", type=str, default=default_model_path,
                        help="基础模型路径")
    parser.add_argument("--data_path", type=str, default=default_data_path,
                        help="训练数据路径（tuneForAbstract.json）")
    ...
    if not os.path.exists(args.model_path):
        ...
        print("请检查模型文件夹是否位于正确路径")
        return

    if not os.path.exists(args.data_path):
        print(f"错误: 数据路径不存在: {args.data_path}")
        print(f"请确保tuneForAbstract.json文件存在")
    ...
    print(f"提示词模板: 简洁解释（3-5句话）")
    print(f"使用设备: {'GPU' if torch.cuda.is_available() else 'CPU'}")

    # 创建训练器并开始训练
    trainer = GPUOptimizedLoRATrainer(args.model_path, args.data_path)
    result = trainer.train(args.output_dir)

    if result:
        print(f"\n=== 训练成功完成 ===")
        print(f"简洁解释LoRA适配器保存在: {result}")
    else:
        print("\n=== 训练失败 ===")
