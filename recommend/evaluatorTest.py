...
class Evaluator:
    ...
    def calculate_pss(
        ...
        ▪ threshold: 相似度阈值（不再用于二值化，仅作保留参数）
        ...
        for i in range(len(context_items)):
            # 获取当前上下文与所有回复块的最大相似度（直接使用相似度值，不进行二值化）
        ...
        logger.info(f"PSS计算完成: 使用{len(max_similarities)}个上下文项的平均相似度")
        return pss_score
    ...
    def calculate_cra(
        ...
        for i in range(len(causal_factors)):
            ...
            causal_similarities.append(sim)

        cra_score = float(np.mean(causal_similarities)) if causal_similarities else 0.0
        logger.info(f"CRA计算完成: 使用{len(causal_similarities)}个因果因素的平均相似度")
        return cra_score
