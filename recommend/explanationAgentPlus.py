...
class ExplanationAgent:
    ...
    def __init__(self, recommender, profile_builder,
        ...
        self.memory_cache = self._init_memory_cache()  # 内存缓存系统
        self.planning_module = self._init_planning_module()  # 规划模块
    ...
    def _init_memory_cache(self):
        ...
        return {
            ...
            'movie_info_cache': {},  # 电影信息缓存
            'explanation_cache': {},  # 解释结果缓存
            'planning_history': {},  # 规划历史记录
            'last_updated': time.time()  # 最后更新时间
        }
    def _init_planning_module(self):
        """初始化规划模块"""
        return {
            'active_sessions': {},  # 活跃会话
            'step_templates': self._get_step_templates(),  # 步骤模板
            'max_steps': 10  # 最大步骤数
        }
    ...
    def generate_explanation(self, user_id, movie_title, explanation_type, predicted_rating):
        ...
        try:
            # === 新增：规划阶段 ===
            session_id = f"{user_id}_{movie_title}_{explanation_type}"
            planning_context = self._start_planning_session(session_id, user_id, movie_title, explanation_type)

            # 步骤1: 获取电影信息（带缓存）
            movie_info = self._get_movie_info_with_cache(movie_title, planning_context)
            if not movie_info:
                return self._get_fallback_explanation(explanation_type)

            # 记录规划步骤
            self._record_planning_step(session_id, "movie_info_retrieved",
                                       {"title": movie_info.get('title'), "source": movie_info.get('source')})

            # 步骤2: 生成提示词（带规划上下文）
            prompt = self._generate_prompt_with_planning(user_id, movie_info, explanation_type,
                                                         predicted_rating, planning_context)

            # 记录规划步骤
            self._record_planning_step(session_id, "prompt_generated",
            ...
            # 记录规划完成
            self._record_planning_step(session_id, "explanation_generated",
                                       {"explanation_length": len(explanation)})

            # === 新增：缓存结果 ===
            self._cache_explanation_result(user_id, movie_title, explanation_type, explanation)
...
【要求】
- 直接输出最终解释，不要思考过程
- 类型名称必须使用英文（如Action, Drama, Sci-Fi），不要翻译成中文
- 语言亲切自然，像朋友间的对话
- 控制在3-5句话内
- 突出个性化匹配点
...
请生成完整的Mermaid代码："""
    ...
    def _start_planning_session(self, session_id, user_id, movie_title, explanation_type):
        """开始规划会话"""
        planning_context = {
        ...
        self.planning_module['active_sessions'][session_id] = planning_context
        return planning_context

    def _get_movie_info_with_cache(self, movie_title, planning_context):
        """带缓存的电影信息获取"""
        cache_key = movie_title.lower().strip()

        # 检查缓存
        if cache_key in self.memory_cache['movie_info_cache']:
            cached_info = self.memory_cache['movie_info_cache'][cache_key]
            # 检查缓存是否过期（1小时）
            if time.time() - cached_info['timestamp'] < 3600:
                planning_context['cached_data_used'] = True
                print("✅ 使用缓存的电影信息")
        ...
        if movie_info:
            # 更新缓存
            self.memory_cache['movie_info_cache'][cache_key] = {
                'data': movie_info,
                'timestamp': time.time()
            }
    ...
    def _generate_prompt_with_planning(self, user_id, movie_info, explanation_type,
                                       predicted_rating, planning_context):
        """带规划上下文的提示词生成"""
        # 获取用户偏好（带缓存）
        preferences, weights = self._get_user_preferences_with_cache(user_id, planning_context)

        # 使用原有的提示词生成逻辑，但传入规划上下文
        prompt = self._generate_prompt(user_id, movie_info, explanation_type, predicted_rating)

        # 添加规划信息到提示词（可选）
        if planning_context['cached_data_used']:
            prompt += "\n# 规划信息：使用缓存数据加速生成"

        return prompt

    def _get_user_preferences_with_cache(self, user_id, planning_context):
        """带缓存的用户偏好获取"""
        cache_key = f"prefs_{user_id}"

        if cache_key in self.memory_cache['user_preferences']:
            cached_data = self.memory_cache['user_preferences'][cache_key]
            if time.time() - cached_data['timestamp'] < 3600:  # 1小时缓存
                planning_context['cached_data_used'] = True
                return cached_data['preferences'], cached_data['weights']

        # 缓存未命中，重新获取
        preferences = self._get_user_preferences(user_id)
        weights = self._get_preference_weights(user_id, preferences)

        # 更新缓存
        self.memory_cache['user_preferences'][cache_key] = {
    ...
    def _record_planning_step(self, session_id, step_name, step_data):
        """记录规划步骤"""
        if session_id in self.planning_module['active_sessions']:
            planning_context = self.planning_module['active_sessions'][session_id]
            planning_context['steps_completed'].append({
                'step': step_name,
                'data': step_data,
                'timestamp': time.time()
            })
            planning_context['current_step'] += 1

    def _cache_explanation_result(self, user_id, movie_title, explanation_type, explanation):
        """缓存解释结果"""
        cache_key = f"{user_id}_{movie_title}_{explanation_type}"
        self.memory_cache['explanation_cache'][cache_key] = {
    ...
    def _get_step_templates(self):
        """获取步骤模板"""
        return {
            1: ['analyze_preferences', 'match_features', 'generate_abstract'],
            2: ['deep_analysis', 'collect_evidence', 'build_argument', 'generate_detailed'],
            3: ['process_analysis', 'extract_steps', 'generate_flow']
        }

    def get_memory_stats(self):
        """获取内存使用统计"""
        return {
            'user_preferences_count': len(self.memory_cache['user_preferences']),
            'movie_info_cache_count': len(self.memory_cache['movie_info_cache']),
            'explanation_cache_count': len(self.memory_cache['explanation_cache']),
            'active_sessions_count': len(self.planning_module['active_sessions']),
            'last_updated': self.memory_cache['last_updated']
        }

    def clear_memory_cache(self):
        """清空内存缓存"""
        self.memory_cache['user_preferences'].clear()
        self.memory_cache['movie_info_cache'].clear()
        self.memory_cache['explanation_cache'].clear()
    ...
    def _get_evaluation_context(self, user_id: int, movie_title: str, explanation_type: int = None) -> List[str]:
        ...
        try:
            ...
            # 用户历史序列
            user_sequence = self._get_user_sequence(user_id, 3)
            if user_sequence:
                context_items.append(f"用户最近观看了{len(user_sequence)}部电影")
                for i, movie in enumerate(user_sequence[:2]):
            ...
            context_items.extend([
                f"电影《{movie_info.get('title', 'Unknown')}》",
                f"导演：{movie_info.get('director', 'Unknown')}",
                f"电影类型：{', '.join(movie_genres)}"
            ])

            # 类型匹配分析
            if preferences and movie_genres:
                matching_genres = set(preferences) & set(movie_genres)
                if matching_genres:
                    context_items.append(f"类型匹配：{', '.join(matching_genres)}")
                else:
                    context_items.append("类型匹配：无直接匹配")

        except Exception as e:
            print(f"构建评估上下文失败: {str(e)}")
            context_items = ["用户观影历史", "电影类型特征", "个性化推荐因素"]

        print(f'统一英文的context_items:', context_items)
    ...
    def _get_evaluation_causal_factors(self, user_id: int, movie_title: str, explanation_type: int = None) -> List[str]:
        ...
        try:
            ...
            if not preferences or not movie_info:
                return ["个性化匹配影响推荐", "电影特征影响用户决策"]
        ...
        print(f'统一英文的causal_factors:', causal_factors)
